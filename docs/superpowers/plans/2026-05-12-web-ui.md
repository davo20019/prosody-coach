...
**Architecture:** Thin web shell (`web/` package) that orchestrates existing domain modules. New `coach_pipeline.py` exposes a single provider-aware `analyze_session(...)` entry the route handlers call. Browser captures webm/opus, server transcodes to WAV with ffmpeg, runs Praat first then the AI coach (sequential — both Gemini and local providers consume the prosody analysis as input, so they can't be concurrent), persists with UUID-named recordings. No CDN scripts, no Node/build step.
