...
Run: `pytest tests/web/test_routes_practice.py -v`
Expected: 8 passed (4 GET + 4 POST: ok-path, coach-fail, transcode-fail, praat-fail).
...
def _passed(prosody, level_config: dict, ai_rhythm_result=None) -> bool:
    ...
    When AI rhythm coaching produced a result, trust its `level_passed` field
    (the model considers tempo, stress contrast, and word-level issues holistically).
    Otherwise fall back to the measured rule: rhythm score meets min_rhythm_score
    AND nPVI is at least npvi_target. Both keys are scalars in
    config.RHYTHM_LEVEL_CONFIG[level].
    """
    if ai_rhythm_result is not None:
        return bool(getattr(ai_rhythm_result, "level_passed", False))
