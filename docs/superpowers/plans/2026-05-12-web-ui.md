...
def test_drills_index_lists_due_drills(client, monkeypatch):
    """Spec parity: Drills shows 'due drills'."""
    ...
    monkeypatch.setattr(
        ...
        lambda level=None, limit=5: [
            {"drill_id": "d-99", "text": "BUH-buh-BUH-buh",
             "level": 1, "last_attempted": "2026-05-10"},
        ],
    )
    response = client.get("/drills")
    assert response.status_code == 200
    assert "BUH-buh-BUH-buh" in response.text
    assert "/drills/level/1" in response.text  # practice link to that level
      ...
      <table>
        ...
        {% for d in due %}
          <tr>
            ...
            <td><a href="/drills/level/{{ d.level }}">practice</a></td>
          </tr>
        {% endfor %}
      </table>
    {% else %}
...
Run: `pytest tests/web/test_routes_drills.py -v`
Expected: 8 passed (index, due-drills, drill page, baseline + 4 attempt scenarios).
