...
def test_drill_attempt_passes_when_ai_says_so(client, tmp_path, monkeypatch):
    """Gemini path: trust rhythm_result.level_passed."""
    ...
    rhythm = SimpleNamespace(
        transcript="x", encouragement="great", technique_tip="lift",
    ...
    assert response.status_code == 200
    assert cap["updates"][0]["passed"] is True  # AI judgment overrode measured fail
...
def test_drill_attempt_local_provider_falls_back_to_measured_rule(client, tmp_path, monkeypatch):
    """Local path: returns CoachingResult (no level_passed) → use measured rule."""
    ...
    # Simulate local path by patching analyze_with_local_coach with a plain
    # CoachingResult-shaped object that has no level_passed attribute.
    local_result = SimpleNamespace(
        transcript="x", coaching_tips=["t"], overall_feedback="ok",
...
def test_drill_attempt_records_failure_when_coach_raises(client, tmp_path, monkeypatch):
    monkeypatch.setattr("web.routes.drills.COACH_PROVIDER", "gemini")
    prosody = _drill_prosody_stub(score=8, pvi=60)
    ...
    assert cap["updates"][0]["passed"] is True  # measured rule still applies
...
progress storage from storage.py. Pass criterion mirrors the CLI: an attempt
"passes" when the measured nPVI is within the level's npvi_target range and
the rhythm component score is >= the level's min_score (both from
RHYTHM_LEVEL_CONFIG, surfaced via progress.levels[level].config). Mastery
evaluation (evaluate_mastery_with_ai) is out of v1 scope.
"""
...
async def attempt(
    ...
    # 3) Determine pass and persist progress. Use AI judgment when available;
    #    fall back to the measured rule.
    progress = get_rhythm_progress()
    ...
    npvi = getattr(prosody.rhythm, "pvi", 0)
    update_rhythm_progress(level=level, npvi=npvi, rhythm_score=prosody.rhythm.score, passed=passed)
    save_rhythm_drill_attempt(
        drill_id=drill_id, level=level,
        npvi=npvi, rhythm_score=prosody.rhythm.score, passed=passed,
    )

    # 4) Persist the session row. Project the rhythm coaching result into the
    #    existing session columns (transcript / ai_summary / ai_tips).
    save_session(
        prosody,
        mode=mode,
        prompt_id=drill_id,
        recording_path=str(wav_path),
        transcript=getattr(rhythm_coaching, "transcript", None) if rhythm_coaching else None,
        ai_summary=getattr(rhythm_coaching, "encouragement", None) if rhythm_coaching else None,
        ai_tips=[getattr(rhythm_coaching, "technique_tip", "")] if rhythm_coaching and getattr(rhythm_coaching, "technique_tip", "") else None,
        coach_provider=COACH_PROVIDER,
        coach_status=coach_status,
        coach_error=coach_error,
    )

    # 5) Render the same analysis_card the practice page uses, with a small
    #    badge indicating pass/fail.
    coach_for_template = None
    if rhythm_coaching is not None:
        coach_for_template = {
            "transcript": getattr(rhythm_coaching, "transcript", None),
            "summary": getattr(rhythm_coaching, "encouragement", None),
            "tips": [t for t in [getattr(rhythm_coaching, "technique_tip", "")] if t],
        }
...
Run: `pytest tests/web/test_routes_drills.py -v`
Expected: 7 passed (index, drill page, baseline + 4 attempt scenarios).
